...
class weixin extends AWS_ADMIN_CONTROLLER
{
    ...
    public function third_party_login_action()
    {
        $this->crumb(AWS_APP::lang()->_t('微信第三方登录'), 'admin/weixin/third_party_login/');
        ...
        $rule_list = $this->model('openid_weixin_thirdlogin')->get_third_party_login_by_account_id($_GET['id']);

        TPL::assign('rule_list', $rule_list);

        TPL::assign('menu_list', $this->model('admin')->fetch_menu_list(809));

        TPL::output('admin/weixin/third_party_login');
    }
    
    
    public function edit_third_party_login_action()
    {
        $this->crumb(AWS_APP::lang()->_t('接入帐号编辑'), 'admin/weixin/third_party_login/');

        if ($_GET['id'])
        {
            $rule_info = $this->model('openid_weixin_thirdlogin')->get_third_party_login_by_id($_GET['id']);

            if (!$rule_info)
            {
                H::redirect_msg(AWS_APP::lang()->_t('不存在第三方微信登录'), '/admin/weixin/reply/');
            }
        ...
        TPL::output('admin/weixin/edit_third_party_login');
    }
