...
*/
    ...
    public function save_third_party_login_action()
    {
        ...
        H::ajax_json_output(AWS_APP::RSM(array(
            'url' => get_js_url('/admin/weixin/third_party_login/id-' . $_POST['account_id'])
        ), 1, null));
    }
    
    public function remove_third_party_login_action()
    {
        if (!$_POST['id'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择要删除的规则')));
        ...
        if(!$this->model('openid_weixin_thirdlogin')->get_third_party_login_by_id($_POST['id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('第三方不存在')));
        }

        $this->model('openid_weixin_thirdlogin')->remove_third_party_login_by_id($_POST['id']);
